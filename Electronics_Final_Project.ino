 ...
 *  
 *  This project was completed as the final project of our Fall 2017 Electronics class. RIP our first light strand, you
 *  will not be forgotten. Wiring schematic can be found here: http://loganswartz.com/resources/20171211_161803.jpg
 */
...
void playCMajor(){
/* takes readings from two rangefinders and creates
 * a note and a chord based on the white keys of a 
 * piano. Plays the note every loop, and the chord
 * every other loop.
 */
  m.lcd.clear();
  bool chordStatus = 0; 
  ...
  while(1){
    ...
    if (buttons == BUTTON_SELECT){
      if (chordStatus == 0){
        chordStatus = 1; // chord and note mode 
      }
      else{
        chordStatus = 0; // note-only mode 
      }
    }
    int root = map(constrain(getRange3(), 0, 100), 0, 100, 0, 29); // maps rangefinder readings to the length of the whiteKeys array; creates a root note for a chord to reference in the 'whiteKeys' array
    int note = map(constrain(getRange2(), 0, 100), 0, 100, 0, 29); // does the same as the previous line on a different rangefinder; will only be used for a single note
    if (root < 29){
      if (chordStatus == 0){ // if you are in note only mode 
        noteOn(whiteKeys[note]); // send the midi controller the hex value that corresponds to the 'noteth' element of 'whitKeys'
      }
      else{ // if you are in chord and note mode 
        if (cycle == 0){ // no chord on this beat
          noteOn(whiteKeys[note]);
          cycle = 1;
        }
        else{ // chord and note on this beat
          noteOn(whiteKeys[note]);
          fullChord(root); // send 3 hex values of the chord generated in fullCord() based on the created root
          cycle = 0;
        }
      }
    }
    int tempoDelay = map(tempo, 60, 240, 1000, 250);
    delay(tempoDelay/2); // use the tempo you set
...
void playPent() {
  /*
   * this was created before playCMajor, so it is a bit more clunky. The general use 
   * of this function is the same as playCMajor, but it just takes notes from the pentatonic
   * scale, rather than C Major. See playCMajor for details
   */
  bool cycle = 0;
  m.lcd.clear();
  while(1){
    ...
    if (note < 26 && chord < 26){
       ...
       *               |
       *  finds a base note, then finds
       *  the two notes above and below
       *  to create a pentatonic chord
       */ 
      else{
        noteOn(pentatonic[note]);
        cycle = 0;
 ...
 */
  ...
  while(1) {
    ...
    //m.lcd.print(range);
    //m.lcd.print("     ");
    colorFollower(createColor(), range); // uses createColor to display a color on the lit leds determined by colorFollower
    noteOn(whiteKeys[createRoot()]); // plays a note generated by createRoot, which reads from rangefinder 3
    delay(50);
    byte buttons = m.getButtons();
    if (buttons == BUTTON_LEFT) {
      return;
...
uint32_t createColor(){
  // creates a color based on rangefinder input for use in other functions
  uint32_t colorRange;
  /*
  * Takes rangefinder values up to 50 centimeters aways
  * and divides that 50 cm range into 5 separate sections.
  * in each section, values will be mapped to 0-255, for 
  * ease of controlling rgb values. 
  */
  ...
  if (range <= 20){
    colorRange = Color(rgbBase1, 0, 0); // only values of red in the first section
  }
  else if (range <= 40){
    colorRange = Color(255, rgbBase2, 0); // add green to get orange and yellow hues
  }
  else if (range <= 60){
    colorRange = Color(255-rgbBase3, 255, 0); // red goes away to get pure green
  }
  else if (range <= 80){
    colorRange = Color(0, 255, rgbBase4); // start adding blues
  } 
  else if(range <= 100){
    colorRange = Color(0, 255-rgbBase5, 255); // green goes away for pure blue
  }
  return colorRange;    
}
...
uint32_t rangeColor(){
  // same as createColor(), but in a self-contained function.
  m.lcd.clear();
  uint32_t colorRange;
  while(1){
...
void noteOn(int pitch) {
  // sends a single note to the midi controller
  Serial.write(0x90);
  Serial.write(pitch);
  Serial.write(0x45);
}
...
void noteOff() {
  Serial.write(80); // turns whatever is playing off
}

int createRoot(){
  /*
   * creates a note by mapping rangefinder input to values that
...
void arpegChord(int root){
  /*does the same as fullChord, but adds delays
...
/* takes one value (the root note) from playCMajor
 *  and sends the corresponding diatonic triad to
 *  the midi controller
 */
  Serial.write(0x90); // sets the comm channel
  Serial.write(whiteKeys[root]); // sends the pitch
  Serial.write(0x35); // determines 'velocity' similar to volume
  Serial.write(0x90);
  Serial.write(whiteKeys[root + 2]);
  Serial.write(0x35);
  Serial.write(0x90);
